...
def rdp_pts(xy0,epsilon=1, nodes=4, run_lim=1000):
    ...
    while simplified.shape[0] != nodes:
        ...
        if run>run_lim:
            return simplified_save, 'two'
        run+=1
    return simplified, 'three'

def rdp_pts_optimize(xy0,a, nodes):
    """
    function to optimize epsilon to create 3 point simplification.
    Local optimization
    """
    def seg_diff(a, xy0, nodes):
        simplified = rdp2(xy0, epsilon=a)
        print(len(simplified)-nodes)
        return np.abs(nodes-len(simplified))

    
    res = minimize(seg_diff, a, method='nelder-mead',
                args=(xy0, nodes), options={'xatol': 0.01})
    eps = res.x

    simplified = rdp2(xy0, epsilon=eps)
    return simplified

def rdp_pts_optimize_g(xy0, nodes):
    """
    function to optimize epsilon to create 3 point simplification.
    Global optimization
    """
    ...
    results = optimize.brute(seg_diff, bounds, args=(xy0, nodes))
...
def simplify_contour(contour, n_corners=4):
    ...
    while True:
        ...
        k = (lb + ub)/2.
        eps = k*cv2.arcLength(contour, closed=False)
...
def simplify_contour_2(contour, n_corners=5):
    import cv2
    '''
    Binary searches best `epsilon` value to force contour 
        approximation contain exactly `n_corners` points.
    ...
    while True:
        ...
        k = (lb + ub)/2.
        _,L = path_length(np.array(contour[:,0]), np.array(contour[:,1]))
        x = np.array(contour[:,0])
        L = np.max(np.abs(x-x[0]))
        eps = k*L
        approx = rdp2(contour, epsilon=eps)
